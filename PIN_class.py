...
class PIN:              # Define PIN class
    ...
    def __init__(self,pin):         # pin is an integer identifying the pin number (assume BOARD mode numbering
        ...
        self.__bounce       = 0
        self.__edge         = "NONE"             # NEED to fix this to valid initial value
    ...
    def set_SL_BOUNCE(self,widget):
        self.__SL_bounce = widget

    def get_SL_BOUNCE(self):
        return __SL_bounce
...
#
    ...
    def set_direction(self, dir):
        if dir == PIN.INPUT_TXT:                    # If input
            ...
            # Call GPIO.setup(self.get_id(),GPIO.OUT)
            # Call GPIO.output(self.__id,val) *** Low or high
            # Call GPIO to Disable PWM
            # Call GPIO to turn event off detection If already on)
            # Call GPIO to set Bounce to default (.1?)
            # set self.__direction
            # set self.__status   = PIN.LOW            
            # set self.__pwm      = PIN.PWM_OFF
            # set self.__freq     = 0
            # set self.__dcyc     = 0
            # set self.__event_det = PIN.ED_OFF    
            # set self.__bounce   = 0
            # Set GUI to OUT
            # Set GUI to High
            # Set GUI PWM off
            # Reset GUI FREQ/DCYC
            # Set GUI edge to none
            # Set GUI bounce to default

            print("Pin", self.__id, "OUT")
            self.__direction = dir                  # Set __direction variable
            ...
            print("Enabling PWM widgets - Pin ", self.__id)
